...
class AiChatViewModel @Inject constructor(
    ...
    fun sendMessage(userMessage: String) {
        ...
        viewModelScope.launch {
            ...
            // Local DeepSeek mode: Use ONNX Runtime for local inference
            try {
                // For now, use fallback until ONNX model is ready
                val response = generateLocalDeepSeekResponse(userMessage)
                val aiMsg = ChatMessage(
                    id = "${messageId}_response",
                    text = response,
                    isUser = false
                ...
                handleError(e.message ?: "Local DeepSeek error", messageId)
            }
        }
    }
    ...
    private fun generateLocalDeepSeekResponse(userQuery: String): String {
        val query = userQuery.lowercase()

        return when {
            query.contains("list") && query.contains("asset") -> """
                <h3>Asset Inventory Query Results</h3>
                <p>Based on local DeepSeek R1 analysis:</p>
            ...
            query.contains("location") || query.contains("room") -> """
                ...
                <p>DeepSeek R1 analysis of location structure:</p>
            ...
            query.contains("movement") || query.contains("transfer") -> """
                ...
                <p>DeepSeek R1 analysis of movement data:</p>
