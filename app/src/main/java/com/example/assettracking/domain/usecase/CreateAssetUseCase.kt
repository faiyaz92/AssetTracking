...
class CreateAssetUseCase @Inject constructor(
    ...
    suspend operator fun invoke(code: String, name: String, details: String?, baseRoomId: Long?): Result<Unit> {
        return if (assetRepository.createAsset(code, name, details, baseRoomId)) {
            Result.success(Unit)
        } else {
